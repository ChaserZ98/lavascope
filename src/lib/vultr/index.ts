...
import { Endpoint, IParams, IVultrAPI } from "./types";
...
export class VultrAPI implements IVultrAPI {
    ...
    private createRequestFunction<PType extends IParams, RType = undefined>(
        endpoint: Endpoint
            ...
            if (!response.ok) {
                return response
                    ...
                    .then((json) => {
                        if (json.error) {
                            throw new Error(
                                `${response.statusText}: ${json.error}`
                            );
                        }
                        throw new Error(response.statusText);
                    });
