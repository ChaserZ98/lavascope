...
function RelativeTime({ date }: { date: string }) {
  ...
  if (diff < 3600) {
    ...
    return <Plural value={minutes} one="# minute ago" other="# minutes ago" />;
  }
  if (diff < 86400) {
        ...
        <ModalBody>
          ...
          <div className="text-warning">
            ...
            <p>
              ...
              <span className="font-mono capitalize">{rule.source}</span>
            </p>
            <p>
              <span>
  ...
  const handleCreate = useCallback(
    async (groupId: string, newRule: CreateRule) => {
      try {
        ...
        const { firewall_rule: rule } = await vultrAPI.firewall.createRule({
          "firewall-group-id": groupId,
          ...newRule,
        });
        logging.info(
        ...
        logging.info(`Successfully added the new rule to local database.`);
        setRules((state) => {
          state[groupId][rule.id] = {
            rule: {
              ...newRule,
    ...
    if (shouldUpdateFromDB) {
      const restoreRulesFromDB = async () => {
        try {
          ...
          logging.error(`Failed to fetch firewall rules from DB: ${err}`);
        } finally {
          setGroups((state) => {
            state[groupId].refreshing = false;
          });
    ...
    <div className="flex flex-col px-8 pb-4 gap-4 items-center select-none">
      ...
      <div className="flex w-full items-center justify-center gap-4">
        ...
        <div className="text-xs md:text-sm">
          ...
          >
            <span className="font-mono">
              {new Date(group.date_created).toLocaleString(language, {
                timeZoneName: "short",
                hour12: false,
              })}
            </span>
      ...
      <div className="flex items-center gap-4">
        <div className="flex flex-col gap-2 items-center">
          ...
          <p className="font-mono text-sm md:text-base">
            {group.description ? group.description : <Trans>[Empty]</Trans>}
          </p>
        </div>
        <Divider
      ...
      <div className="flex flex-col w-full relative bg-content1 p-4 overflow-auto rounded-large shadow-small shadow-content1 transition-[color,background-color,border-color,text-decoration-color,fill,stroke,opacity,box-shadow] ease-[ease] duration-250 max-w-fit min-h-60">
        <Tabs
          ...
          classNames={{
            ...
            tabList: "transition-colors-opacity bg-content2 my-auto",
      ...
      <div className="flex gap-4 justify-center items-center flex-wrap">
        <Button onPress={() => handleRefresh(groupId)} isLoading={refreshing}>
          <Trans>Refresh</Trans>
        </Button>
        <ProxySwitch />
      </div>
